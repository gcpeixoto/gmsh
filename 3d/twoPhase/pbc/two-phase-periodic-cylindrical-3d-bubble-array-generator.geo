/* File: periodic-cylindrical-3d-bubble-array-generator.geo
...
//B = 12*rCap; // body length
DH = B + rCap; 
//DH = B + 4/3*rCap + rCap; 
g = DH/2; // distance from inlet and outlet 
s = DH/2; // slug length
//g = (16/3)*rCap; // distance from inlet and outlet 
//s = (16/3)*rCap; // slug length
...
p2 = newp;
Point(p2) = {xMin,yMin,zMin - r1,wallPeriodic}; 
p3 = newp;
Point(p3) = {xMin,yMin + r1,zMin,wallPeriodic}; 
p4 = newp;
Point(p4) = {xMin,yMin,zMin + r1,wallPeriodic};
p5 = newp;
Point(p5) = {xMin,yMin - r1,zMin,wallPeriodic};

// right end
p6 = newp;
Point(p6) = {xMin + L,yMin,zMin,wallPeriodic}; // center
p7 = newp;
Point(p7) = {xMin + L,yMin,zMin - r1,wallPeriodic}; 
p8 = newp;
Point(p8) = {xMin + L,yMin + r1,zMin,wallPeriodic}; 
p9 = newp;
Point(p9) = {xMin + L,yMin,zMin + r1,wallPeriodic};
p10 = newp;
Point(p10) = {xMin + L,yMin - r1,zMin,wallPeriodic};
...
nt = 2; // number of theta points per semi-semi circle (total around circle is 4*nt) 
...
// Origin
x0 = xMin;
//x0 = xMin+0.6*rCap; // to fit long bubble
y0 = yMin;
z0 = zMin;
...
/* --- DISCRETIZATION (THETA) CIRCLES --- */
nt2 = 2; // number of theta points per semi-semi circle (total around circle is 4*nt) 
...
//Transfinite Surface {s1} Alternate;
//Transfinite Surface {s2} Alternate;
...
// Wall's surfaces
Physical Surface("wallInvU") = {s3,s4,s5,s6};
Physical Surface("wallL") = {s1};
Physical Surface("wallR") = {s2};

// Bubbles' surfaces mounted according Printf of the loop (output do Printf está ERRADO!)
Physical Surface("bubble1") = {30, 46, 40, 42, 34, 44, 36, 38, 32};
Physical Surface("bubble2") = {64, 66, 68, 70, 72, 74, 76, 78, 80};
Physical Surface("bubble3") = {98, 100, 102, 104, 106, 108, 110, 112, 114};
//Physical Surface("bubble4") = {132, 134, 136, 138, 140, 142, 144, 146, 148};
